...
void transpose(std::vector<int>& mtx, int n, int m)
{
	std::vector<int> transposed;

	for (int j = 0; j < m; ++j)
		for (int i = 0; i < n; ++i)
			transposed.push_back(mtx[i * m + j]);
...
int main(int argc, char* argv[])
{
	...
	{
		if (rank == 0)
			std::cout << "Infezabil: Nr. de procese e mai mult decat numarul de coloane." << std::endl;
	...
	if (rank == 0)
	{
		...
		std::cout << std::endl;
		transpose(mtx, sizes[0], sizes[1]);
	}

	int part = sizes[1] / ranks;

	if (rank + 1 == ranks)
		part = sizes[1] - rank * part;

	int effectiveSize = sizes[0] * part;
